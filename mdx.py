    ...
    def __init__(self, model_path: str, params: MDXModel, processor=DEFAULT_PROCESSOR):
        ...
        if "CUDAExecutionProvider" in available_providers:
            ...
            if self.device.type == "cuda":
                warnings.warn("CUDAExecutionProvider not available in ONNXRuntime; falling back to CPU.")
...
def run_mdx(
    ...
    if mp is None:
        raise RuntimeError(f"Parameters for model hash {model_hash} not found in model_data.json")
    model = MDXModel(
        device,
